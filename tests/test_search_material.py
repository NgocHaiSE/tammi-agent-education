...
from agent.graph.node_services.search_material.search_material import SearchMaterialNode
...
def mock_llm():
    ...
    llm = AsyncMock()
    llm.ainvoke.return_value = AIMessage(content="Đây là tài liệu học Python cơ bản.")
    return llm
...
def mock_api_gateway_client():
    ...
    client.tavily_search = AsyncMock(return_value={
        "data": {
            "results": [
                {"title": "Python Basics", "url": "http://example.com/python-basics", "content": "Nội dung về Python cơ bản.", "score": 0.9},
            ],
            "answer": "Python là ngôn ngữ lập trình phổ biến.",
        }
    })
    return client
...
def mock_settings():
    ...
    with unittest.mock.patch('agent.graph.node_services.search_material.search_material.get_settings', return_value=settings_mock):
        yield

@pytest.mark.asyncio
async def test_search_material_run(mock_llm, mock_api_gateway_client):
    """Test the run method of SearchMaterialNode."""
    # Mock the APIGatewayHTTPClient context manager
    with unittest.mock.patch('agent.external_clients.http_client.APIGatewayHTTPClient', return_value=mock_api_gateway_client):
        node = SearchMaterialNode(llm=mock_llm)
        
        state = GraphState(
            request={
                "payload": {"content": "Tìm tài liệu học python cơ bản"},
        ...
        assert "Đây là tài liệu học Python cơ bản." in result["node_responses"][0].content
        
        assert "data_artifacts" in result
        assert "retrieval_summary" in result["data_artifacts"]
        assert result["data_artifacts"]["retrieval_summary"]["query"] == "Tìm tài liệu học python cơ bản"
        ...
        mock_api_gateway_client.tavily_search.assert_called_once_with(
            query="Tìm tài liệu học python cơ bản",
...
@pytest.mark.asyncio
async def test_search_material_run_no_tavily_results(mock_llm, mock_api_gateway_client):
    ...
    with unittest.mock.patch('agent.external_clients.http_client.APIGatewayHTTPClient', return_value=mock_api_gateway_client):
        ...
        assert "node_responses" in result
        assert isinstance(result["node_responses"][0], AIMessage)
        assert "Đây là tài liệu học Python cơ bản." in result["node_responses"][0].content # LLM still responds
...
@pytest.mark.asyncio
async def test_search_material_run_exception_in_tavily(mock_llm, mock_api_gateway_client):
    ...
    with unittest.mock.patch('agent.external_clients.http_client.APIGatewayHTTPClient', return_value=mock_api_gateway_client):
        ...
        assert "node_responses" in result
        assert isinstance(result["node_responses"][0], AIMessage)
        assert "Xin lỗi, tôi gặp khó khăn trong việc tìm kiếm tài liệu." in result["node_responses"][0].content
